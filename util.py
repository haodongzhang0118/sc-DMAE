...
class AverageMeter(object):
    ...
    def update(self, loss, n=1):
        val = float(loss['total_loss'].detach().cpu().numpy())
        self.val = val
        self.sum += val * n
        self.count += n
        self.avg = self.sum / self.count

        self.reconstruction_loss = float(loss['reconstruction_loss'].detach().cpu().numpy())
        self.reconstruction_loss_sum += self.reconstruction_loss * n
        self.mask_loss = float(loss['mask_loss'].detach().cpu().numpy())
        self.mask_loss_sum += self.mask_loss * n
        self.latent_loss = float(loss['latent_loss'].detach().cpu().numpy())
        ...
        self.weight_r = loss['weight_r']
        self.weight_m = loss['weight_m']
        self.weight_l = loss['weight_l']
